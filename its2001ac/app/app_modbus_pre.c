...
void app_modbus_pre_task(void)
{
    ...
    if(cfged)
    {
        ...
        rd_stauts = pull_mRtu_register(MODBUS_PRE,&reg_addr,&reg_length);
        if(mRtuS_read == rd_stauts)
        {
            while(reg_length)
            {
                reg_detailes = app_modbus_read_reg_data(reg_addr);           
                if(push_mRtu_readReg(MODBUS_PRE,reg_addr,reg_detailes))
                {
                    reg_addr++;
                    reg_length--;
                }
        ...
        else if(mRtuS_write == rd_stauts)
        {
            while(reg_length)
            {
                ...
                if(pull_mRtu_writeReg(MODBUS_PRE,reg_addr,&rd_wReg_details))
                {
        ...
        else if(mRtuS_rwBoth == rd_stauts)
        {
            ...
            pull_mRtu_register_wb(MODBUS_PRE,&w_reg_addr,&w_reg_length);

            while(w_reg_length)
            {
                sdt_int16u rd_wReg_details;
                if(pull_mRtu_writeReg(MODBUS_PRE,w_reg_addr,&rd_wReg_details))
                {
                    app_modebus_write_reg_data(reg_addr,rd_wReg_details);
                    w_reg_addr++;
                    w_reg_length--;
    ...
    else
    {
        ...
        set_mRtu_parameter(MODBUS_PRE,&parameter);
    }
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
